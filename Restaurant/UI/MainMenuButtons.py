...
class Dashboard(QWidget):
    ...
    def configure_buttons(self, layout):
        ...
        for i, label in enumerate(button_labels):
            ...
            elif label == "Live Ordering System":
                button.clicked.connect(self.open_los)
            elif label == "Reservation System":
                button.clicked.connect(self.open_reservationsystem)
    ...
    def open_management(self):
        try:
            ...
            script_path = os.path.abspath("../Management/ManagementMenu.py")
            # Run the script using subprocess.Popen
            subprocess.Popen(["python", script_path])

        except FileNotFoundError:
            print("Unable to find ManagementMenu.py.")
    ...
    def open_reservationsystem(self):
        try:
            # Get the absolute path to the script
            script_path = os.path.abspath("../Management/Make_Reservations.py")
            # Run the script using subprocess.Popen
            subprocess.Popen(["python", script_path])

        except FileNotFoundError:
            print("Unable to find Make_Reservations.py.")
