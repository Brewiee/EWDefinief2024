...
class SupplierManagementApp(QMainWindow):
    def __init__(self):
        ...
        # Hide new supplier fields initially
        self.new_supplier_fields_widget.setVisible(False)

    def init_ui(self):
        self.layout = QVBoxLayout()  # Define QVBoxLayout as instance variable

        # Add checkbox to toggle new supplier fields
        self.new_supplier_checkbox = QCheckBox("New Supplier")
        self.new_supplier_checkbox.stateChanged.connect(self.toggle_supplier_fields)
        self.layout.addWidget(self.new_supplier_checkbox)

        # Add new supplier fields layout
        self.new_supplier_fields_widget = QWidget()
        self.new_supplier_fields = QVBoxLayout(self.new_supplier_fields_widget)
        ...
        # Modify the layout of new supplier fields to set fixed width for labels and dynamic width for input fields
        for icon_filename, field_label in zip(icon_filenames, field_labels):
            ...
            input_width = 1000  # You can adjust this value as needed
            field_input.setMinimumWidth(input_width)  # Set minimum width for input fields
            ...
            self.new_supplier_fields.addLayout(layout_row)
            self.line_edit_fields[field_label] = field_input  # Add line edit field to dictionary

        self.layout.addWidget(self.new_supplier_fields_widget)

        self.btn_save = QPushButton("Save New Supplier")
        self.btn_save.clicked.connect(self.save_new_supplier)  # Connect to save_new_supplier method
        ...
        # Print Button
        self.btn_print_pdf = QPushButton("Print Supplier List to PDF")
        self.btn_print_pdf.clicked.connect(self.print_suppliers_to_pdf)
    ...
    def populate_table(self):
        try:
            with self.conn.cursor() as cursor:
                cursor.execute("SELECT * FROM Supplier")  # Fetch all rows from Supplier table
                suppliers = cursor.fetchall()

                self.table.setRowCount(len(suppliers))
                for row, supplier in enumerate(suppliers):
                    for col, field in enumerate(
                            ["CR_Supplier_Supplier_ID", "CR_Supplier_Name", "CR_Supplier_Email",
                             "CR_Supplier_Country", "CR_Supplier_Address", "CR_Supplier_Zipcode",
                             "CR_Supplier_City", "CR_Supplier_Phone_number", "CR_Supplier_VAT"]):
                        if field == "CR_Supplier_Supplier_ID":
                            item = NumericTableWidgetItem(supplier[field])
                        else:
                            item = QTableWidgetItem(str(supplier[field]))
                            if field == "CR_Supplier_City":
    ...
    def save_new_supplier(self):
        try:
            # Get supplier details from the input fields
            ...
            if not name or not email or not country or not address or not postal_code or not city or not phone or not vat:
            ...
            for widget in self.new_supplier_fields_widget.findChildren(QLineEdit):
                widget.clear()
            self.populate_table()
            QMessageBox.information(self, "Success", "New supplier added successfully.")
        except pymysql.Error as e:
            QMessageBox.warning(self, "Error", f"Error saving new supplier: {str(e)}")

    def get_line_edit_text(self, label_text):
        for i in range(self.new_supplier_fields.count()):
            layout = self.new_supplier_fields.itemAt(i)
    ...
    def toggle_supplier_fields(self):
        ...
        if self.new_supplier_checkbox.isChecked():
            self.new_supplier_fields_widget.setVisible(True)
    ...
    def print_suppliers_to_pdf(self):
        try:
            # Supplier FPDF class to include header
            class PDF(FPDF):
                def header(self):
            ...
            with self.conn.cursor() as cursor:
                # Fetch all rows from Supplier table
                cursor.execute("SELECT * FROM Supplier")
                suppliers = cursor.fetchall()

                # Filter the suppliers based on search input
                search_text = self.search_input.text().strip().lower()
                if search_text:
                    sorted_suppliers = [supplier for supplier in suppliers if
                                 any(search_text in str(field).lower() for field in supplier.values())]
                else:
                    sorted_suppliers = sorted(suppliers, key=lambda x: x['CR_Supplier_Supplier_ID'])
                ...
                for supplier in sorted_suppliers:
                    for i, field in enumerate(supplier.values()):
            ...
            reply = QMessageBox.information(self, "Success",
                                            f"Supplier list saved to {pdf_output_path}. Do you want to open it?",
        ...
        try:
            with self.conn.cursor() as cursor:
                for row in range(self.table.rowCount()):
                    ...
                    vat = self.table.item(row, 8).text()
                    ...
                    cursor.execute("""
                        ...
                        SET 
                            CR_Supplier_Name = %s, 
                            CR_Supplier_Email = %s,
                            CR_Supplier_Country = %s,
                            CR_Supplier_Address = %s,
                            CR_Supplier_Zipcode = %s,
                            CR_Supplier_City = %s,
                            CR_Supplier_Phone_number = %s,
                            CR_Supplier_VAT = %s
                        WHERE 
                            CR_Supplier_Supplier_ID = %s
                    """, (
